                    ...
                    <div className="py-[80px] max-md:py-[60px]">
                        <div className="max-[860px]:w-[80vw] w-[450px] bg-white py-10 max-[860px]:px-2 shadow-lg rounded-2xl">
                            <div className='px-8 mb-3 flex flex-col items-center text-center'>
                                ...
                                <h3 className="pt-8 mb-3 text-md font-bold">{t("Please verify your email")}</h3>
                            ...
                            <div className="mt-4 flex justify-between items-center gap-3">
                                {showCountdownText && (
                                    <p className="text-sm px-8 pb-4">{t("Resend the code in")} {countdown} {t("seconds")}</p>
                                )}
                                <form onSubmit={handleSubmit2(ResendCode)} className="px-8 pb-4 bg-white rounded">
                                    <div className="text-center flex items-center gap-2">
                                        <button
                                            id="submit"
                                            type="submit"
                                            className={`w-fit font-bold bg-[var(--secondary)] !text-[var(--primary)] rounded-full transition-all delay-100 p-2 text-[15px] ${canResend ? '' : 'opacity-50 pointer-events-none'}`}
                                            disabled={!canResend}
                                        >
                                            <GrPowerReset />
                                        </button>
                                        {!showCountdownText && (
                                            <label htmlFor="submit" className={`${canResend ? 'text-[var(--primary)] hover:text-gray-400 cursor-pointer font-semibold' : 'text-gray-400'}`}>{t("Resend the code")}</label>
                                        )}
                                    </div>
                                </form>
                            </div>
                            ...
                            >
                                <div className="mb-6 text-center">
                                    <label className="block mb-4 text-sm text-start font-bold text-gray-700" htmlFor="email">
                                    ...
                                    />
                                    {errors.verificationcode && <span className='text-red-500 text-xs mt-1 ms-2'>{t("Validation code Error")}</span>}
                                </div>
