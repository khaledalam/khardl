...
const Model = ({ buttonId }) => {
    ...
    const [selectedResturent, setSelectedResturent] = useState('');
    ...
    useEffect(() => {
        ...
        fetchDataFromCities();
          ...
          // API GET REQUEST For Resturents
      useEffect(() => {
        const fetchDataFromResturents = async () => {
          try {
            const response = await fetch('');
            const jsonData = await response.json();
            setResturent(jsonData);
          } catch (error) {
            console.error('', error);
          }
        };
    
        fetchDataFromResturents();
      }, []);
                    ...
                    <div className='flex flex-col justify-start items-start gap-2 gap-y-3 flex-wrap'>
                        ...
                        <div className="w-[100%]">
                            ...
                            <div className='relative'>
                              ...
                              >
                                {cities.map((city) => (
                                  <option className="bg-white text-black" key={city.id} value={city.value}>
                                    {city.label}
                                  </option>
                                ))}
                              </select>
                        ...
                        <div className="w-[100%]">
                            ...
                            <div className='relative'>
                                ...
                                id="resturent"
                                className='text-[14px] bg-[var(--secondary)] w-[100%] p-2 rounded-full px-4 appearance-none'
                                value={selectedResturent}
                                onChange={(e) => setSelectedResturent(e.target.value)}
                              >
                                {resturents.map((resturent) => (
                                  <option className="bg-white text-black" key={resturent.id} value={resturent.value}>
                                    {resturent.label}
                                  </option>
                                ))}
                              </select>
