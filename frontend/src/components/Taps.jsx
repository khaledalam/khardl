...
export function Taps({ children, contentClassName = "" }) {
  ...
  return (
    <>
      <div className={`flex px-2 justify-evenly items-center gap-4 bg-[var(--secondary)] ${contentClassName}`}>
        {children.map((item, i) => {
          return (
            <>
              {TapValidator(item) && (
              ...
              {(i !== children.length - 1) && template === "resturents" &&(
  ...
  return (
    <>
      <div
       ...
       text-[18px] max-[600px]:text-[15px] ${activeTap === currentTap ? `font-bold border-b-[3px] border-b-[var(--primary)]` : ""} 
