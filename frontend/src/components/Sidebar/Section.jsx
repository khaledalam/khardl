...
import { selectCategory, getSelectedCategory } from '../../redux/editor/categorySlice';
import { selectAlign, getSelectedAlign } from '../../redux/editor/alignSlice';
import { selectBanner, getSelectedBanner } from '../../redux/editor/bannerSlice';
import { updateIconInput, setSelectedIconId, updatePhoneNumber, moveFromMoreToIcons, moveFromIconsToMore } from '../../redux/editor/contactSlice.js';
        ...
        <div className='flex flex-col justify-between'>
            <div className=''>
                <div className='flex justify-between items-center p-4'>
                    <div className='flex items-center gap-2 '>
                        <div className='text-[18px] font-semibold'>{t("Logo")}</div>
                        <input type="file" accept="image/*" id="Logo" onChange={handleLogoChange} className="hidden" />
                        <label
                            htmlFor="Logo"
            ...
            <div className='p-4 pt-6'>
                <div className='flex justify-between items-center'>
                    <div className='text-[18px] font-semibold'>{t("Social Media")}</div>
                    {more_icons.length > 0 &&
                        ...
                        {more_icons.map((icon) => {
                            ...
                            return (
                                <div
                                    ...
                                    onClick={() => handleMoveFromMoreToIcons(icon.id)}
                                >
                            ...
                            >
                                ...
                                {icon.id === selectedIconId && (
                                    <button
                                        ...
                                        onClick={() => handleMoveFromIconsToMore(icon.id)}
                                    >
                                        <AiOutlineClose />
                                    </button>
                                )}
                    ...
                    <div key={icon.id}>
                        {icon.id === selectedIconId && (
                            <>
                                <input
                                    type="text"
                                    placeholder={Language == "en" ? icon.name + ` ${t("Link")}` : `${t("Link")} ` + icon.name}
                                    value={icon.Link}
                                    onChange={(e) =>
                                        dispatch(updateIconInput({ id: icon.id, Link: e.target.value }))
                                    }
