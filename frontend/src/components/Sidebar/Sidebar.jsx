...
const Sidebar = () => {
  ...
  return (
    <div position={position} className={`bg-white !text-black ${position === 'static' ? "!min-w-[18%] !max-w-[18%] pt-[100px]" : "pt-[20px]"}  text-white text-xl h-[100vh] overflow-y-auto shadow-md`}>
      {template === "resturents" &&
        <Taps contentClassName="bg-[#ffffff15] w-[100%]">
          <Tap component={<Section />} active>{t("Section")}</Tap>
          <Tap component={<Edit />}>{t("Edit")}</Tap>
        </Taps>
      }
      {template === "customers" &&
        <Taps contentClassName="bg-[#ffffff15] w-[100%] !justify-start !px-6">
        <Tap component={<Edit />} contentClassName="!px-[0px]" active >{t("Edit")}</Tap>
          <Tap></Tap>
        </Taps>
      }
    </div>
