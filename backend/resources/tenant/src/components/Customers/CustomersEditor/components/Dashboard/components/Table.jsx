...
function GlobalFilter({ globalFilter, setGlobalFilter }) {
    ...
    return (
        <div className="relative w-[100%]">
            <div
                className={`text-gray-300 absolute inset-y-0 ${Language == "en" ? "right-0" : "left-0"} flex items-center pe-3 pointer-events-none`}
            >
                <BiSearch />
            </div>
            <input
...
export function SelectColumnFilter({
    ...
    return (
        <label
            className={`flex justify-between gap-x-2 items-center relative w-[100%]`}
        >
            <div
                className={`text-gray-300 absolute inset-y-0 ${Language == "en" ? "right-0" : "left-0"} flex items-center pe-3 pointer-events-none`}
            >
    ...
    return (
        <>
            {activeTab === "Orders" && (
                <div>
                    <div
                        className={`flex justify-between gap-3 ${divWidth <= 366 ? "flex-wrap !justify-start items-center gap-2" : ""}`}
                    >
                        <GlobalFilter
                    ...
                    <p className="font-bold flex items-center justify-start mt-6">
                        <span>
                            {preFilteredRows.length <= 10 ? (
                                <span>
                                    {preFilteredRows.length} {t("orders")}
                                </span>
                            ) : (
                                <span>
                                    {preFilteredRows.length} {t("Order")}
                                </span>
                            )}
                        </span>
                    </p>
            ...
            {activeTab === "Orders" && (
                <div className="py-3 flex items-center justify-between">
                    ...
                    <div
                        className={`hidden sm:flex-1 sm:flex sm:items-center sm:justify-between gap-2 ${divWidth <= 366 ? "flex-wrap !justify-center items-center mt-4 gap-4" : ""}`}
                    >
                        <div className="flex gap-x-5 items-center">
                            {preGlobalFilteredRows.length >= 5 && (
                                <label
                                    className={`flex justify-between gap-x-2 items-center relative`}
                                >
                                    <div
                                        className={`text-gray-300 absolute inset-y-0 ${Language == "en" ? "right-0" : "left-0"} flex items-center pe-3 pointer-events-none`}
                                    >
