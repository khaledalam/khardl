...
const DetailesItem = ({ onClose }) => {
  ...
  const GlobalColor = sessionStorage.getItem('globalColor');
    ...
    let selectedBranch = branches.filter(b => b?.id == localStorage.getItem('selected_branch_id'));
    ...
    const saveDefaultBranchLocal = () => {
        ...
        localStorage.setItem('selected_branch_id', branchesSelectRef.current?.value);
                  ...
                  <div className="modal max-w-md min-w-[480px] bg-white overflow-y-auto mx-5 xl:max-w-xl lg:max-w-xl md:max-w-xl max-h-screen shadow-lg flex-row rounded-lg ">
                      <div className="modal-header grid grid-cols-1 p-5 items-center border-b border-ternary-light">
                          ...
                          <select defaultValue={selectedBranch?.id} ref={branchesSelectRef} className='text-[14px] bg-[var(--primary)] w-[100%] p-1 rounded-full px-4 appearance-none'>
                              {/*<option value={null}/>*/}
                              {branches?.map((b, idx) => {
                                  return <option key={idx} value={b?.id}>{b?.name}</option>;
                              })}
                          </select>
                          ...
                          <div className="flex justify-center items-center">
                              <button
                                  ...
                                  style={{backgroundColor: "var(--primary)", borderRadius: GlobalShape}}
                              >
                                  {t("Save")}
                              </button>
                          </div>
