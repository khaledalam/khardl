...
const Editor = () => {
  ...
  return (
    <div ref={divRef} className="w-[100%] bg-white h-[85vh] overflow-y-auto" style={{ fontFamily: `${selectedFontFamily || styleDataRestaurant?.font_family}`, fontWeight: `${selectedFontWeight || styleDataRestaurant?.font_type}`,fontSize:`${selectedFontFamily || styleDataRestaurant?.font_size}` }}>
                            ...
                            return <Tap
                                        ...
                                        component={
                                            category?.items?.length > 0 ? <MenuItems
                                                    items={category?.items?.filter(item => item?.availability == '1')} // only availabile items
