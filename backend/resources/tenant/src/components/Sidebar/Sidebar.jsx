    ...
    const handleSaveStyle = async (e) => {
        ...
        if (template === "restaurants") {
            inputs.logo = state?.logo
                ? await fetch(state?.logo).then((r) => r.blob())
                : "";
            ...
            inputs.banner_image = state?.image
                ? await fetch(state?.image).then((r) => r.blob())
                : "";
            if (state?.images?.image && state?.images?.image.length > 0) {
                const imagePromises = state?.images?.image.map(
                    async (image) => {
            ...
            {template === "restaurants" && (
                <>
                    ...
                    <div className={"flex  justify-around w-[100%]"}>
                        <Link
                            className={
                                "flex justify-center content-center bg-[var(--primary)] w-[50%]"
                            }
                        ...
                        <Link
                            className={
                                "flex border-[red] justify-center content-center bg-[var(--primary)] w-[50%]"
                            }
                            to={"/"}
                            target={"_blank"}
                            href={"/"}
