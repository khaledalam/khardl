...
const CustomerOrder = () => {
  ...
  const [pageNumber, setPageNumber] = useState(
    useSelector((state) => state.customerAPI.currentPage)
  );
  const { totalCount } = useSelector((state) => state.customerAPI);
  ...
  const fetchOrderPerpage = async () => {
    try {
      const ordersResponse = await AxiosInstance.get(
        `orders?items&item&per_page=${12}&page=${pageNumber}&search=${search}&status=${orderStatus}`
      );
      ...
      <div className="flex flex-wrap gap-4 mb-5 mx-3 min-h-96">
        {ordersList?.map((order, index) => (
          <OrderItem
            key={index}
            order={order}
            onClick={() => setSearchParams({ orderId: order.orderId })}
          />
        ))}
        {ordersList?.length === 0 && (
