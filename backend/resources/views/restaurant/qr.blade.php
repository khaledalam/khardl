                    ...
                    <!--begin::Item-->
                        @foreach ($qrcodes as $qrcode)
                            ...
                            <div class="d-flex align-items-center mb-8">
                                ...
                                <div class="d-flex">
                                    <a href="{{ route('restaurant.qr-download', $qrcode->id) }}" class="btn btn-icon btn-bg-light btn-active-color-khardl btn-sm me-1">
                                        <i class="fas fa-download"></i>
                                    </a>
                                    @if($user->isRestaurantOwner())
                                    <form class="delete-form" id="delete_qr_{{ $qrcode->id }}" action="{{ route('restaurant.qr-delete', ['id' => $qrcode->id]) }}" method="POST">
                                        @method('DELETE')
                                        @csrf
                                    </form>
                                    <a href="#" onclick='deleteQR("{{ $qrcode->id }}")' class="delete-button btn btn-icon btn-bg-light btn-active-color-khardl btn-sm">
...
<script>
    ...
    function deleteQR(QRId) {
        ...
        var form = document.getElementById(`delete_qr_${QRId}`);
        Swal.fire({
            title: `{{ __("Are you sure you want to delete this QR ?") }}`
            , icon: 'warning'
