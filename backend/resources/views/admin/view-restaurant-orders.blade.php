														...
														confirmButtons.forEach(function(button) {
															button.addEventListener('click', function(event) {
																...
																Swal.fire({
																	...
																	inputOptions: {
																		'option1': '{{ __('messages.commercial-registration-number') }}',
																		'option2': '{{ __('messages.delivery-company-contract') }}',
																		'option3': '{{ __('messages.both') }}'
																	},
																	...
																	if (result.isConfirmed) {
																		...
																		form.insertAdjacentHTML('beforeend', '<input type="hidden" name="option" value="' + selectedOption + '">');
																		form.submit();
																	}
																});
															});
                        ...
                        <div class="d-flex justify-content-between align-items-center">
                            ...
                            <ul class="nav nav-stretch nav-line-tabs nav-line-tabs-2x border-transparent fs-5 fw-bolder">
                                ...
                                <li class="nav-item mt-2">
                                    <a href="{{ route('admin.view-restaurants', ['id' => $restaurant->id]) }}" class="nav-link text-active-primary ms-0 me-10 py-5">{{ __('messages.overview') }}</a>
                                </li>
                                <!--end::Nav item-->
                                <!--begin::Nav item-->
                                @if ($restaurant->isApproved == 1)
                                    ...
                                    <li class="nav-item mt-2">
                                        <a  class="nav-link text-active-primary ms-0 me-10 py-5 active" href="{{ route('admin.view-restaurants-orders', ['id' => $restaurant->id]) }}">{{ __('messages.orders') }}</a>
                                    </li>
                                    <!--end::Nav item-->
                                    <!--begin::Nav item-->
                                    <li class="nav-item mt-2">
											...
											<table class="table table-flush align-middle table-row-bordered table-row-solid gy-4 gs-9">
												...
												<thead class="border-gray-200 fs-5 fw-bold bg-lighten">
													<tr>
														...
														<th class="min-w-150px px-0">{{ __('messages.order-id') }}</th>
														<th class="min-w-350px">{{ __('messages.details') }}</th>
												...
												<tbody class="fs-6 fw-bold text-gray-600">
                                                    @foreach ($orders as $order)
                                                        <tr>
                                                            <td class="ps-9">{{ $order->date }}</td>
                                                            <td class="ps-0">{{ $order->order_id }}</td>
                                                            <td>{{ $order->product_detail }}</td>
                                                            <td class="text-success">{{ $order->price }}</td>
                                                            <td class="text-center">
                                                                <button class="btn btn-light btn-sm btn-active-light-primary">{{ __('messages.download') }}</button>
                                                            </td>
                                                        </tr>
                                                    @endforeach
												</tbody>
