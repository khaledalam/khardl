...
use App\Jobs\SendCustomerNotifyForNewSub;
use Carbon\Carbon;
use Closure;
use Exception;
use Throwable;
...
class RestaurantCharge
{
    ...
    {
        if(app()->getLocale() == 'ar')$date = Carbon::now()->translatedFormat('l j F Y');
        else $date = Carbon::now()->format('Y-m-d');
    ...
    public static function NotifyCustomer($data)
    {
        $user = RestaurantUser::first();
        if(app()->getLocale() == 'ar')$date = Carbon::now()->translatedFormat('l j F Y');
        else $date = Carbon::now()->format('Y-m-d');
        SendCustomerNotifyForNewSub::dispatch(
            $user,
            Setting::first()->restaurant_name,
            tenant()->id,
            $data['amount'],
